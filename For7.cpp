...
void AddData(Player& tempDataHolder) //Acessing Struck player
{
...
void AddRecord(Player& tempDataHolder, Node*& head)
{
	...
	temp->playerData = tempDataHolder;
...
void OpenFile(Node*& head) {
	...
	while (current != nullptr) {
		outFile << "Name:: " << current->playerData.nickname << std::endl;
		outFile << "Age: " << current->playerData.age << std::endl;
		outFile << "Scores:";
		for (int i = 0; i < 5; ++i) {
			outFile << current->playerData.scores[i] << " ";
		}
		outFile << std::endl;
