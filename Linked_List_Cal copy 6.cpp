...
struct DataHolder { // This will be a 
    // Member Variable
...
int main() {
    ...
    while (true) {
        ...
        case 1:
            do
            {
                choice = MathMenu(); //Calculation you will see 

                // if choice is 0(false) then it will be converted to true then will break
                if (!choice) break;

                InputValue(temp); //Getting the history...

                Computation(choice, temp); // Using a passing by reference(Max)...
...
int HistoryMenu(int currentPage, int lastPage, int historyCounter) { //creates an instruction in the case, history counter will will be key here, 
...
void DisplayHistory(History *head, int currentPage, int lastPage) {
    ...
    int counter = 0, startingIndex = (currentPage - 1) * 10, i = 0; //Page
    ...
    if (!head)
        ...
        while (head && counter < 10) { //Caping of the output in History 
...
int HistoryFunctionalities(int &historyCounter, History *&head, History *&tail) {
    ...
    int currentPage = 1, lastPage = (historyCounter - 1) / 10 + 1; //Page
    ...
    while (currentPage <= lastPage) {
        ...
        case 3:
            UpdateHistory(head);
            system("pause");
            return 0; // Return 0 to repeat the program
        case 4:
            std::cout << "2";
            system("pause");
            return 0;
...
int UpdateHistory(History *head) {
    int position;
    double new_val1, new_val2, new_result;
    std::cout << "Enter position to update: ";
    std::cin >> position;
    ...
    while (current != nullptr) {
        if (count == position) {
            std::cout << "Enter new value for input 1: ";
            std::cin >> new_val1;
            std::cout << "Enter new value for input 2: ";
            std::cin >> new_val2;
            std::cout << "Enter new result: ";
            std::cin >> new_result;

            // Update the data
            current->data->val[0] = new_val1;
            current->data->val[1] = new_val2;
            current->data->result = new_result;

            std::cout << "\nNode at position " << position << " updated." << std::endl;
    ...
    return 0; // Return 0 to indicate unsuccessful update
}
